...
def setup_cmd() -> None:
    ...
    print("  uv sync --extra yolo           # optional (Ultralytics + OpenCV)")
...
# -----------------------------
# CV (YOLO) specific commands
# -----------------------------
def train_cmd() -> None:
    raise NotImplementedError(
        "Training is optional for this starter. "
        "If you want, add a dataset and call Ultralytics train from scripts/train.py."
    )


def eval_cmd() -> None:
    raise NotImplementedError(
        "Evaluation is project-specific. Add scripts/eval.py to compute FPS/mAP/etc."
    )


def predict_cmd() -> None:
    """
    Run real-time YOLO inference on webcam/video/image.
    Requires optional extra: `uv sync --extra yolo`.
    """
    import sys
    ...
    ap.add_argument("--source", default="0", help="0 for webcam, or path/URL for video/image.")
    ap.add_argument("--model", default="yolo11n.pt", help="Model weights (e.g., yolo11n.pt).")
    ap.add_argument("--imgsz", type=int, default=640, help="Inference image size.")
    ap.add_argument("--conf", type=float, default=0.25, help="Confidence threshold.")
    ap.add_argument("--save", action="store_true", help="Save outputs to outputs/")
    ...
    out_dir = cfg.outputs_dir / "yolo"
    out_dir.mkdir(parents=True, exist_ok=True)

    model = YOLO(args.model)
    print(f"[ok] model={args.model} source={args.source} imgsz={args.imgsz} conf={args.conf}")
    # Ultralytics handles webcam/video/image sources.
    # save=True will write to default 'runs/' unless project sets it.
    # Here we use project outputs by setting project root as the run directory.
    ...
    # results is a list of Results; we just print summary.
    print(f"[done] predict items={len(results)} outputs={out_dir}")
