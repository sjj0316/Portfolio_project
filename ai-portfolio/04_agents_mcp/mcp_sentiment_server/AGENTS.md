...
You are working in a small portfolio project (**Sentiment MCP Server**).
...
## What / Why
- Minimal sentiment MCP server with a heuristic tool, health resource, and prompt template.

## How to test (local)
- [ ] `uv sync --extra dev`
- [ ] `uv run smoke --profile local`
- [ ] (If server changes) `uv run test`
