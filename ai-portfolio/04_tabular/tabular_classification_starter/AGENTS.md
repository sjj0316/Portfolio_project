...
You are working in a small portfolio project (**Tabular Classification Starter**).
...
## What / Why
- Lightweight tabular demo with optional scikit-learn baseline and pure-Python fallback.

## How to test (local)
- [ ] `uv sync --extra dev`
- [ ] `uv run smoke --profile local`
- [ ] (If training/eval touched) `uv sync --extra ml` then `uv run train --profile local` and `uv run eval --profile local`
